...
* **`index.html`**: A página de entrada/login.
* **`principal.html`**: O formulário do cidadão para enviar relatos.
* **`admin.html`**: O painel de administração para ver o feed.
* **`script.js`**: Contém a lógica de login do `index.html`.
* **`principal.js`**: Contém a lógica de envio de formulário e geolocalização do `principal.html`.
* **`admin.js`**: Contém a lógica de autenticação, filtros e `fetch` do painel `admin.html`.
* **`style.css` / `admin.css`**: Arquivos de estilização.
* **`/images`**: Contém os assets visuais (logo, imagem de fundo).
